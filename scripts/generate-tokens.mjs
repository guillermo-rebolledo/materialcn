...
/**
 * Material's window size classes. The breakpoints are Material's own, not
 * Tailwind's stock scale — `md` fires at 768px, which lands in the middle of
 * Material's medium class rather than on either of its edges.
 *
 * Each entry is the *lower* bound of a class; its upper bound is the next
 * entry's lower bound, minus one.
 */
const WINDOW_SIZE_CLASSES = [
  ["compact", 0, "Phone in portrait."],
  ["medium", 600, "Tablet in portrait, or an unfolded phone."],
  ["expanded", 840, "Tablet in landscape, or a small desktop window."],
  ["large", 1200, "Desktop."],
  ["extra-large", 1600, "Ultra-wide desktop, or a TV."],
]

const bounds = (i) => {
  const [, min] = WINDOW_SIZE_CLASSES[i]
  const next = WINDOW_SIZE_CLASSES[i + 1]
  return next ? `${min}–${next[1] - 1}dp` : `${min}dp and up`
}
