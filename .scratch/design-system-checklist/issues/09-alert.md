...
- [ ] Supports an optional title, so a longer message can be scanned.
- [ ] Supports a leading icon, so the role is legible to colourblind users
      without relying on the background colour alone.
- [ ] Supports actions related to the message.
- [ ] Announces its role correctly to assistive technology, distinguishing an
      urgent alert from a passive status message.
- [ ] Adapts to narrow viewports, going full-width where the layout calls for it.
- [ ] Exported from the public barrel with stories covering every role, the
