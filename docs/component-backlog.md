...
- [ ] Add non-toggle standard and connected button groups.
- [ ] Add `SplitButton` with a primary action and menu trigger.
- [ ] Support the Material size and shape combinations shown in the kit.
- [ ] Keep selectable segmented controls on the existing `ToggleGroup` rather
...
- [ ] Add `SearchBar` with leading search, trailing actions, active, disabled,
...
- [ ] Add small, medium, and large `TopAppBar` configurations.
- [ ] Add `BottomAppBar` with actions and optional FAB placement.
- [ ] Support scrolling/elevation state without owning the page's scroll
  container.
